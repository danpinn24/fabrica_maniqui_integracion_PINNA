...
let piezas = require('./data/piezas'); // Se mantiene 'let' para poder borrar
...
// 2. CREAR NUEVA PIEZA (POST) - Soporta Cabeza, Torso, Brazos y Piernas
app.post('/api/piezas', (req, res) => {
  ...
  if (datos.tipo === 'Cabeza') {
    ...
    nuevaPieza.piel = datos.piel || nuevaPieza.color; 
...
// 3. ELIMINAR UNA PIEZA (DELETE)
app.delete('/api/piezas/:id', (req, res) => {
  const idBorrar = parseInt(req.params.id);
  piezas = piezas.filter(p => p.id !== idBorrar);
  res.json({ mensaje: "Pieza eliminada correctamente del stock" });
});
